...
{
	const rngArgs = msg.content.split(" ").shift();
	//min value is index 0
	//max value is index 1

	return msg.channel.send(rngArgs);

	if(rngArgs[0] || rngArgs[1] == undefined) return msg.channel.send("you must give me the min and max values for the rng to work");

	var no_name = Math.round(Math.random() * max) + min + "\u200b";
